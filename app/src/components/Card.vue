<template>
  <div :class="{'zoom': !isPressed, 'card': true}" :id="'card'+cardId" :style="{backgroundImage: 'url('+image+')'}" @mousedown="mouseIsPressed" @mouseup="mouseIsReleased()">
    <!-- this is card -->
  </div>
</template>
...
<style scoped>
    .card {
        ...
        bottom: 0px;
        transition: width .15s ease-in-out, height .15s ease-in-out, margin .25s ease-in-out;
        /* position: absolute; */
        /* bottom: 0; */
        /* margin: 0px -30px; */
        margin-bottom: -146px;
